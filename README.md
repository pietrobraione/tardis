...
* JBSE needs to interact with an external SMT solver for pruning infeasible program paths. At the purpose TARDIS uses [Z3](https://github.com/Z3Prover/z3), that comes in the shape of standalone binaries for all the platforms that can be installed almost everywhere in your system.
...
* `-evosuite` (command line) or `setEvosuitePath` (`tardis.Options`): the path to the EvoSuite jar file `evosuite-shaded-<version>-SNAPSHOT.jar` contained in the `libs/` folder. It must be the same jar file you put in the classpath (see previous section).
* `-jbse_lib` (command line) or `setJBSELibraryPath` (`tardis.Options`): the path of the JBSE jar file from the `jbse/build/libs` directory. It must be the same jar file you put in the classpath. If you chose to deploy the TARDIS uber-jar `tardis-master-<VERSION>-shaded.jar`, set this option to point to it.
