...
public class Node implements Cloneable, Serializable {
	//SUSHI: INSTRUMENTATION BEGIN
	//Triggers
	  @SuppressWarnings("unused")
	  private static void _handleLastNull(Node node) {
		  if(!Analysis.isResolved(node, "first")) return;
		  Analysis.assume(node.first == null);
	  }
	  @SuppressWarnings("unused")
	  private static void _handleChildrenListClosure(Node node) {
		 if(!Analysis.isResolved(node, "last")) return;
		 Node lastChild = null;
		 for(Node n = node.first; n != null; n = n.next)
			  lastChild = n;
		 Analysis.assume(node.last == lastChild);
	  }
	  @SuppressWarnings("unused")
	  private static  void _gotLastExpand(Node parentNode) {
		  nodeTypes(parentNode.last, parentNode);
		  if(!Analysis.isResolved(parentNode, "first")) return;
		  for(Node n = parentNode.first; n != null; n = n.next){
			  if(!Analysis.isResolved(n, "next")) {
				  return;
			  }
		  }
	  ...
	  private static void nodeTypes(Node newNode, Node parent) {		  
		  Analysis.assume(!(parent instanceof Node.StringNode));
		  if (newNode instanceof Node.StringNode) {
			  Analysis.assume(
					  newNode.type == Token.NAME ||
					  newNode.type == Token.STRING);
		  }
	  }

	  @SuppressWarnings("unused")
	  private static  void _gotFirstExpand(Node parentNode) {
		  nodeTypes(parentNode.first, parentNode);
	  }

	  @SuppressWarnings("unused")
	  private static  void _gotNextExpand(Node parentNode) {
		  Analysis.assume(parentNode.type == Token.FUNCTION);
		  nodeTypes(parentNode.next, parentNode);
	  }

	  @SuppressWarnings("unused")
	  private static  void _gotRootExpand(Node rootNode) {
		  Analysis.assume(rootNode.type == Token.BLOCK);
		  nodeTypes(rootNode, null);
	  }
	  
	  @SuppressWarnings("unused")
	  private static  void _gotStrExpand(Node.StringNode strNode) {
		  Analysis.assume(strNode.str.length() == 0 || strNode.str.hashCode() != 0);
	  }

//SUSHI: INSTRUMENTATION END
  ...
  private static final String propToString(int propType) {
      switch (propType) {
        ...
        default:
          throw new IllegalStateException("unexpect prop id " /*SUSHI: + propType*/);
      }
  }
  ...
  public int getExistingIntProp(int propType) {
    PropListItem item = lookupProperty(propType);
    if (item == null) {
      throw new IllegalStateException("missing prop: " /*SUSHI: + propType*/);
    }
    return item.getIntValue();
  }
  ...
  public double getDouble() throws UnsupportedOperationException {
    if (this.getType() == Token.NUMBER) {
      ...
      throw new UnsupportedOperationException(/*SUSHI: this +*/ " is not a number node");
    }
  }
  ...
  public void setDouble(double s) throws UnsupportedOperationException {
    if (this.getType() == Token.NUMBER) {
      ...
      throw new UnsupportedOperationException(/*SUSHI: this +*/ " is not a string node");
    }
  }
  ...
  public String getQualifiedName() {
    if (type == Token.NAME) {
      ...
      return /*SUSHI: left + "." +*/ getLastChild().getString();
